...
The skills in the **Routing Branches** table above are **MANDATORY**, not optional. They are invoked with their full plugin-prefixed names:

- `superpowers:test-driven-development` (was shorthand `tdd-guide`) for L1+ code work
- `superpowers:verification-before-completion` before any "done" claim
- `superpowers:systematic-debugging` for L2+ bug fixes (prompt contains `bug`/`failing`/`broken`/`crash` keywords)
- `superpowers:using-superpowers` at the start of every non-L0 session

The Stop hook `harness/hooks/quality-discipline-stop.{json,js}` (shipped by this plugin) inspects each session's transcript at session-end and warns on stderr if a session completed without invoking the skills its trigger conditions required. See `harness/rules/task-workflow.md` "🔴 Enforcement" section for the full rule table.

**Warn-only.** The hook exits 0 always — it informs, doesn't gatekeep. Tune thresholds after 1-2 self-evolve cycles.
